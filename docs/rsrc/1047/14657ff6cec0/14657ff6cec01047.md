...
Permission denied (public key) になる
...
デフォルトだと UTC になっているので、 JST に変更する

```sh
$ date
Mon Feb  9 05:03:51 UTC 2026
$ sudo timedatectl set-timezone Asia/Tokyo
$ date
Mon Feb  9 14:04:43 JST 2026
```

# hostname の設定と固定化

ホスト名を変えても、再起動時に元に戻る現象があった。

cloud-init を無効化してみたりしたが、結局の解決は `/etc/hostname` を変更することだった

```
sudo nano /etc/hostname
```

ここに `ubuntu` がしぶとく残り続けていたので、希望するホスト名に書き換えたところ、再起動するとこのホスト名に切り替わった